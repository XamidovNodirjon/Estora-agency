    ...
    <style>
        ...
        * {
        ...
        .footer-content {
            ...
            flex-wrap: wrap;
            /* Kichik ekranlarda o'ralash uchun */
        }
        ...
        .footer-logo img {
            height: 55px;
            /* Logotip balandligi kichraytirildi */
            margin-bottom: 5px;
        }
        ...
        .footer-social {
            ...
            /* Bo'shliq kamaytirildi */
            font-size: 1.3em;
            /* Ikonka o'lchami kichraytirildi */
        }
        ...
        .card {
            display: flex;
            align-items: stretch;
            /* Ikkala tomon bir xil balandlikda bo'lishi uchun */
            max-height: 250px;
            /* Kartochkaning umumiy balandligini cheklaymiz */
            overflow: hidden;
            border-radius: 12px;
        }
        ...
        .card-image {
            ...
            /* Rasm umumiy kenglikning 30% qismini egallaydi */
            height: 100% !important;
            object-fit: cover;
            /* Rasmni cho'zmasdan, konteynerga moslab kesadi */
        }
        ...
        .search-form-container {
            ...
            padding: 2rem;
            margin-top: -100px;
            /* Qisman fon ustiga chiqishi uchun */
            position: relative;
            z-index: 10;
        }
            ...
            <div class="search-form-grid">
                <div class="form-group">
                    ...
                    <select name="ad_type" id="ad_type">
                        ...
                        <option value="sale" {{ request('ad_type') == 'sale' ? 'selected' : '' }}>{{ __('Sotish') }}
                        </option>
                        <option value="rent" {{ request('ad_type') == 'rent' ? 'selected' : '' }}>{{ __('Ijaraga') }}
                        </option>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="property_type" id="property_type">
                        ...
                        <option value="land" {{ request('property_type') == 'land' ? 'selected' : '' }}>{{ __('Yer') }}
                        </option>
                ...
                <div class="form-group">
                    ...
                    <select name="rooms" id="rooms">
                        <option value="All">{{ __('Hammasi') }}</option>
                        @for ($i = 1; $i <= 5; $i++)
                            <option value="{{ $i }}" {{ request('rooms') == $i ? 'selected' : '' }}>{{ $i }} {{ __('xona') }}
                            </option>
                        @endfor
                ...
                <div class="more-filters-hidden" style="display: none;">
                    <div class="form-group">
                        ...
                        <select name="floors" id="floors">
                            <option value="All">{{ __('Hammasi') }}</option>
                            @for ($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" {{ request('floors') == $i ? 'selected' : '' }}>{{ $i }}
                                    {{ __('Qavat') }}
                                </option>
                            @endfor
                            <option value="6+" {{ request('floors') == '6+' ? 'selected' : '' }}>6+ {{ __('Qavat') }}
                            </option>
                        </select>
                    </div>
                </div>
            ...
            <div class="filter-actions">
                ...
                <div class="filter-buttons">
                    ...
                    <button type="submit" class="show-ads-btn">
                        <i class="bi bi-search"></i> {{ __('Ko\'rish') }} {{ $filteredProducts->total() }}
                        {{ __('e\'lonlar') }}
                    </button>
                </div>
            </div>
                ...
                <div class="space-y-6">
                    @foreach($filteredProducts as $product)
                        <div class="flex flex-col md:flex-row bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition ad-card"
                            ...
                            <div class="relative md:w-1/3 w-full h-56 md:h-64 open-image-modal" x-data="{ currentIndex: 0 }">
                                @php
                                ...
                                <div class="overflow-hidden w-full h-full relative">
                                    ...
                                    @if(empty($images))
                                        <img src="https://placehold.co/400x300/CCCCCC/333333?text=Rasm+Yo‘q"
                                            class="absolute inset-0 w-full h-full object-cover" alt="No Image">
                                    @endif
                                </div>
                                @if(count($images) > 1)
                                    <button
                                        class="prev-button-card absolute top-1/2 left-3 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow"
                                        @click="currentIndex = (currentIndex === 0) ? {{ count($images) - 1 }} : currentIndex - 1">‹</button>
                                    <button
                                        class="next-button-card absolute top-1/2 right-3 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow"
                                    ...
                                    <div class="absolute bottom-2 left-1/2 -translate-x-1/2 flex space-x-2">
                                        @foreach($images as $index => $img)
                                            <span class="w-2 h-2 rounded-full"
                                                :class="currentIndex === {{ $index }} ? 'bg-yellow-500' : 'bg-gray-300'"></span>
                                        @endforeach
                                    </div>
                                @endif
                            ...
                            <div class="flex-1 p-4 flex flex-col justify-between">
                                <div>
                                    <div class="flex items-center justify-between mb-2">
                                        <span class="text-xs text-white bg-yellow-600 px-2 py-1 rounded-md">ID
                                            {{ $product->id }}</span>
                                        <span
                                            class="text-lg font-bold text-green-700">{{ number_format($product->price, 0, '.', ' ') }}
                                            USD</span>
                                    </div>
                                    <h3 class="text-base font-semibold text-gray-800">
                                        {{ $product->category->name ?? __('Kategoriya yo‘q') }}
                                    </h3>
                                    <p class="text-gray-600 text-sm mt-1 flex items-center gap-1"><i
                                    ...
                                    <p class="text-gray-500 text-sm mt-1">{{ __('Mo‘ljal:') }}
                                        {{ $product->landmark ?? __('Ko‘rsatilmagan') }}
                                    </p>
                                ...
                                <p class="text-gray-400 text-xs mt-3">{{ __('E\'lon joylangan sana:') }}
                                    {{ $product->created_at->format('d.m.Y') }}
                                </p>
                            </div>
                            <div
                                class="md:w-1/4 w-full border-t md:border-t-0 md:border-l border-gray-200 p-4 flex flex-col justify-between">
                                <div>
                                    <a href="tel:+998951606446">
                                        <button
                                            class="w-full bg-blue-600 text-white py-2 rounded-lg font-medium hover:bg-blue-700">{{ __('Telefon raqam') }}</button>
                                    </a>
                                    <button
                                        class="w-full mt-2 bg-gray-200 text-gray-700 py-2 rounded-lg font-medium hover:bg-gray-300 open-contact-modal"
                                        data-product-name="{{ $product->name }}"
                                        data-product-id="{{ $product->id }}">{{ __('Biz bilan bog\'lanish') }}</button>
                                </div>
                                    ...
                                    @if($product->floor > 0)
                                        <span class="flex items-center gap-1 bg-gray-100 px-2 py-1 rounded-md">
                                            <img src="{{ asset('images/4465213.png') }}" alt="icon" class="w-4 h-4"
                                                onerror="this.src='https://placehold.co/20x20/CCCCCC/333333?text=Icon';">
                                ...
                                @if($product->features && $product->features->count())
                                    <div class="flex flex-wrap gap-2 mt-3">
                                        @foreach($product->features as $feature)
                                            <span
                                                class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-md">{{ $feature->feature_name }}</span>
                                        @endforeach
                                    </div>
                                @endif
                                <div class="flex gap-2 mt-3">
                                    @if($product->exchange)<span
                                    class="text-xs border border-yellow-500 text-yellow-600 px-2 py-1 rounded-md">Ayirboshlash</span>@endif
                                    @if($product->pay_in_installments)<span
                                        class="text-xs border border-green-500 text-green-600 px-2 py-1 rounded-md">Bo'lib
                                    to'lash</span>@endif
                                    @if($product->credit)<span
                                    class="text-xs border border-blue-500 text-blue-600 px-2 py-1 rounded-md">Ipoteka</span>@endif
                                </div>
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="modal-content">
            ...
            <form id="contactForm">
                <div class="form-group">
                    ...
                    <input type="text" id="contactName" name="name" placeholder="{{ __('Ismingizni kiriting') }}"
                        required>
                </div>
                <div class="form-group">
