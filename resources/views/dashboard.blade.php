            ...
            <div class="flex justify-between items-center py-2 relative">
                <div class="flex items-center gap-4">
                    <div class="social-icons flex gap-3">
                        <a href="#" class="text-secondary-gold hover:text-primary-gold"><i
                ...
                <div class="test-mode-banner px-4 py-1 rounded-full text-sm flex items-center gap-2 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 animate-blink shadow-sm font-semibold">
                    <i class="fas fa-exclamation-triangle"></i>
                    <span style="text-transform: uppercase; letter-spacing: 1px;">The site is in test mode.</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center py-3 md:py-4">
                ...
                <div class="flex items-center gap-2 md:gap-8">
                    @php
                        $createProductRoute = route('login');
                        if (auth()->guard('web')->check()) {
                            $createProductRoute = route('create-product');
                        } elseif (auth()->guard('client')->check()) {
                            $createProductRoute = route('client.products.create');
                        }
                    @endphp
                    <a href="{{ $createProductRoute }}"
                        class="bg-secondary-gold text-white px-3 md:px-6 py-2 rounded-lg font-medium hover:bg-primary-gold transition-colors flex items-center gap-2 text-sm md:text-base">
                    ...
                    @if(auth()->guard('web')->check() || auth()->guard('client')->check())
                        @php
                            $user = auth()->guard('web')->check() ? auth()->guard('web')->user() : auth()->guard('client')->user();
                            $dashboardRoute = auth()->guard('web')->check() 
                                ? ($user->type === 'super_admin' || $user->username === 'super_admin' ? route('admin-dashboard') : route('products')) 
                                : route('get.client');
                        @endphp
                        <div class="flex items-center gap-4">
                            <a href="{{ $dashboardRoute }}"
                                ...
                                <span class="hidden sm:inline">Kabinet</span>
                            </a>
                            <form action="{{ route('logout') }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="text-gray-700 hover:text-red-500 text-sm md:text-base">
            ...
            <div class="max-w-6xl mx-auto mt-2 px-2 md:px-0">
                <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div class="flex flex-row flex-nowrap overflow-x-auto bg-primary-blue" style="font-size: 14.5px">
                        <button
                            class="tab flex-none text-white px-6 py-3 font-medium hover:text-yellow-300 focus:text-yellow-300">
                            Ijara
                        </button>
                        <button
                            class="tab flex-none text-white px-6 py-3 font-medium hover:text-yellow-300 focus:text-yellow-300">
                    ...
                    <div class="bg-white p-4 md:p-6">
                        <form action="{{ route('products.filter') }}" method="GET">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4 mb-6">
                                <div class="filter-item">
                                    ...
                                    <div class="relative">
                                        <select name="property_type"
                                            class="w-full p-2 text-sm bg-transparent border-0 border-b-2 border-gray-300 focus:border-primary-blue focus:outline-none appearance-none">
                                        ...
                                        <i
                                            class="fas fa-chevron-down absolute right-0 top-2 text-gray-400 text-xs pointer-events-none"></i>
                                    </div>
                                </div>
                                ...
                                <div class="filter-item">
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Vositachilik
                                        haqqi</label>
                                    <div class="relative">
                            ...
                            <!-- Action Buttons -->
                            <div
                                class="flex flex-col md:flex-row gap-3 justify-between items-center pt-3 border-t border-gray-200">
                                <button type="button"
                                    class="w-full md:w-auto bg-secondary-gold text-primary-blue px-5 py-2 rounded-lg font-medium hover:bg-primary-gold hover:text-white transition-colors flex items-center justify-center gap-2 text-sm shadow-md">
                                ...
                                <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                                    <button type="button"
                                        class="w-full md:w-auto border-2 border-primary-blue text-primary-blue px-5 py-2 rounded-lg font-medium hover:bg-primary-blue hover:text-white transition-colors flex items-center justify-center gap-2 text-sm shadow-md">
                                        <i class="fas fa-map-marker-alt"></i>
                                        orqali qidirish
                                    </button>

                                    <button type="submit"
                                        class="w-full md:w-auto bg-primary-blue text-white px-6 py-2 rounded-lg font-medium hover:bg-secondary-blue transition-colors flex items-center justify-center gap-2 text-sm shadow-md">
    ...
    <section class="py-8 md:py-16 bg-gray-50">
        <div class="container mx-auto px-4">
            <div class="text-center mb-8 md:mb-12">
                ...
                <p class="text-gray-600 text-sm md:text-base px-4">Siz uchun eng maqbul va samarali yechimlarni topishda
                    ishonchli hamkoringiz bo'lamiz.</p>
            </div>
            ...
            <div class="relative max-w-7xl mx-auto">
                <!-- Navigation Buttons -->
                <button id="prevBtn"
                    class="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-white shadow-xl text-primary-blue w-12 h-12 rounded-full hover:bg-primary-blue hover:text-white transition-all duration-300 flex items-center justify-center border-2 border-primary-blue">
                    <i class="fas fa-chevron-left text-lg"></i>
                </button>
                <button id="nextBtn"
                    class="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-white shadow-xl text-primary-blue w-12 h-12 rounded-full hover:bg-primary-blue hover:text-white transition-all duration-300 flex items-center justify-center border-2 border-primary-blue">
                            ...
                            <div class="w-full md:w-1/3 flex-shrink-0 px-3" data-index="{{ $index }}">
                                <div
                                    ...
                                    <div class="relative">
                                        ...
                                        <!-- TOP Badge -->
                                        <div
                                            class="absolute top-3 left-3 bg-gradient-to-r from-yellow-400 to-yellow-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                                            <i class="fas fa-star mr-1"></i>TOP
                                        </div>

                                        <!-- Heart Button -->
                                        <button
                                            class="absolute top-3 right-3 bg-white bg-opacity-90 hover:bg-opacity-100 p-2 rounded-full transition-all duration-300 shadow-lg">
                                        ...
                                        <div class="absolute bottom-3 left-3">
                                            ...
                                            @endphp
                                            <span
                                                class="px-3 py-1 rounded-full text-white text-xs font-medium {{ $randomBadge['class'] }}">
                                                {{ $randomBadge['text'] }}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-5">
                                        ...
                                        <div class="flex justify-between items-start mb-3">
                                            <div>
                                                ...
                                                <p class="text-gray-600 font-medium text-sm">
                                                    {{ $product->category->name ?? 'Mulk' }}</p>
                                            </div>
                                            <div class="text-right">
                                                <div
                                                    class="bg-primary-blue text-white px-2 py-1 rounded-lg text-xs font-medium">
                                        ...
                                        <div class="flex items-center text-gray-500 text-sm mb-4">
                                            ...
                                            <span>{{ $product->region->name ?? 'Toshkent' }},
                                                {{ $product->city->name ?? 'Shahar' }}</span>
                                        </div>
                ...
                <div class="flex justify-center mt-6 space-x-2">
                    @for($i = 0; $i < ceil(count($bestOffers) / 3); $i++)
                        <button
                            class="carousel-indicator w-3 h-3 rounded-full transition-all duration-300 {{ $i === 0 ? 'bg-primary-blue' : 'bg-gray-300' }}"
                            data-slide="{{ $i }}"></button>
                    @endfor
                </div>
            </div>
        ...
        <div class="container mx-auto px-4">
            <div class="flex flex-col lg:flex-row items-center gap-8 md:gap-12">
                ...
                <div class="lg:w-1/2 text-center lg:text-left">
                    ...
                    <button
                        class="bg-secondary-gold text-primary-blue font-bold py-3 md:py-4 px-6 md:px-8 rounded-lg shadow-lg hover:bg-primary-gold transition duration-300 flex items-center gap-3 mx-auto lg:mx-0 text-sm md:text-base">
        ...
        <div class="container mx-auto px-4 py-8 md:py-16">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
                <div class="text-center lg:text-left">
                    <img src="{{ asset('logo/footer-logo.png') }}" alt="Estora Logo"
                        class="mb-4 md:mb-6 footer-logo mx-auto lg:mx-0">
                    <p class="text-xs md:text-sm text-footer-primary mb-4">YTT "Estora", 2025 yy. Barcha huquqlar
                        himoyalangan</p>
                    <p class="text-xs text-footer-primary">
                        Saytdan foydalanish orqali <strong>Foydalanuvchi shartnomasi</strong> va
                        <strong>Shaxsiy ma'lumotlarni qayta ishlash siyosati</strong> bilan rozilik bildirganingizni
                        anglatadi.
                    </p>
                </div>
                ...
                <div class="text-center">
                    <div class="mb-4 md:mb-6">
                        <p class="text-footer-primary text-xs md:text-sm mb-2">O'zbekiston bo'ylab barcha qo'ng'iroqlar
                            bepul</p>
                    ...
                    <div class="flex justify-center gap-3 md:gap-4">
                        <a href="https://www.instagram.com/estora.uz/?__pwa=1#"
                            class="text-secondary-gold hover:text-white text-2xl transition-colors">
                ...
                <div class="text-center lg:text-right">
                    <button
                        class="bg-primary-blue bg-opacity-80 text-white px-4 md:px-6 py-2 md:py-3 rounded-full hover:bg-opacity-100 transition-all flex items-center gap-2 mx-auto lg:ml-auto text-sm md:text-base">
            ...
            <div class="text-center text-xs md:text-sm text-footer-primary px-4">
                © 2025 Estora — Barcha huquqlar himoyalangan. estora.uz saytida joylashtirilgan ma'lumotlardan
                foydalanish — jumladan,
                ularni namoyish etish, nusxa ko'chirish, ko'paytirish yoki tarqatish — faqatgina manbaga faol havola
                ko'rsatilgan taqdirda ruxsat etiladi.
            </div>
        </div>
    </footer>
    ...
    <div id="mobile-menu" class="mobile-menu fixed inset-0 bg-white z-50 hidden md:hidden">
        <div class="mobile-menu-content h-full flex flex-col">
            ...
            <nav class="mobile-nav flex-1 p-4 overflow-y-auto">
                <a href="#" class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Bosh
                    Sahifa</a>
                <a href="#" class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Biz
                    Haqimizda</a>
                <a href="#"
                    class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Yangiliklar</a>
                <a href="#"
                    class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Ijara</a>
                <a href="#" class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Sotib
                    olish</a>
                <a href="#"
                    class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">NoBroker</a>
                <a href="#"
                    class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Expats</a>
                <a href="#" class="mobile-nav-link block py-2 text-gray-700 hover:text-secondary-gold border-b">Business
                    Space</a>
            </nav>
