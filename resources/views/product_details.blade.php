                ...
                <nav class="main-nav">
                    ...
                    <div class="language-selector">
                        <select id="language-switcher">
                            ...
                            <option value="uz" {{ app()->getLocale() == 'uz' ? 'selected' : '' }}>{{ __('UZBEK') }}
                            </option>
                            <option value="ru" {{ app()->getLocale() == 'ru' ? 'selected' : '' }}>{{ __('RUSSIAN') }}
                            </option>
                        </select>
                    </div>
    ...
    <section class="product-details-section">
        <div class="container">
            ...
            <div class="product-gallery">
                <div class="image-viewer-container" style="position: relative; max-width: 800px; margin: 0 auto;">
                    @if(count($product->image_array) > 0)
                        <img src="{{ asset('storage/' . $product->image_array[0]) }}" alt="{{ $product->name }}"
                            class="main-image">
                    @else
                        <img src="https://placehold.co/800x600/CCCCCC/333333?text={{ urlencode(__('Rasm Yoq')) }}"
                            alt="{{ __('Rasm Yoq') }}" class="main-image">
                    @endif
                    <button class="nav-button prev-button"
                        style="position: absolute; top: 50%; left: 10px; transform: translateY(-50%); background: #F7931E; color: white; border: none; padding: 10px 15px; border-radius: 50%; cursor: pointer; font-size: 1.5em; z-index: 10; display: flex; align-items: center; justify-content: center;">
                        <i class="bi bi-chevron-left"></i>
                    </button>
                    <button class="nav-button next-button"
                        style="position: absolute; top: 50%; right: 10px; transform: translateY(-50%); background: #F7931E; color: white; border: none; padding: 10px 15px; border-radius: 50%; cursor: pointer; font-size: 1.5em; z-index: 10; display: flex; align-items: center; justify-content: center;">
                        <i class="bi bi-chevron-right"></i>
                    </button>
                </div>
                <div class="thumbnail-images">
                    @foreach($product->image_array as $index => $image)
                        <img src="{{ asset('storage/' . $image) }}" alt="{{ __('Mahsulot rasmi') }}" class="thumbnail"
                            data-index="{{ $index }}">
                    @endforeach
                </div>
            </div>
            ...
            <div class="product-info-grid">
                ...
                <div class="info-block">
                    <h4>{{ __('Manzil') }}</h4>
                    <p>{{ $product->region->name ?? __('Noma\'lum hudud') }},
                        {{ $product->city->name ?? __('Noma\'lum shahar') }}</p>
                </div>
                @if($product->rooms > 0)
                    <div class="info-block">
                ...
                <div class="related-listings-grid">
                    @foreach($relatedProducts as $relatedProduct)
                        <a href="{{ route('products.show', $relatedProduct->id) }}" class="related-listing-card">
                            ...
                            <p class="related-price">{{ number_format($relatedProduct->price, 0, '.', ' ') }} {{ __('y.e.') }}
                            </p>
                        </a>
                    @endforeach
                </div>
